...
.service('VideoTools', function($http) {
    ...
    controller.getVideoThumbnail = function(item, callback) {

        if (!item || !item._id) {
            return callback('No video specified');
        }


        if(thumbCache[item._id]) {
            return callback(null, thumbCache[item._id]);
        }
        ...
        switch (item.assetType) {
            case 'youtube':
                ...
                return callback(null, thumbCachethumbCache[item._id]);
                break;
            case 'vimeo':
                ...
                break;
            case 'upload':
                return callback('No thumbnails available for uploaded videos');
                break;
        }
    }
...
.directive('videoThumbnail', function() {
    ...
    return {
        ...
        template: '<span><img ng-src="{{model.thumbnailUrl}}"/></span>',
